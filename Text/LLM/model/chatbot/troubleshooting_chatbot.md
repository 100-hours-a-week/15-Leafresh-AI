...
| 기능 | 기존 코드 | 현재 코드 (개선) |
| :--- | :--- | :--- |
| **스트리밍 방식** | 복잡한 단어 단위로 분해 | **공백 단위**로 분할하여 자연스러운 띄어쓰기 유지 |
| **챌린지 줄바꿈** | 구분 없이 출력되어 한 줄에 붙는 문제 발생 | 새 챌린지 번호 앞에 **강제로 줄바꿈을 추가**하여 문제 해결 |
| **JSON 파싱** | 전체 텍스트를 대상으로 한 복잡하고 불안정한 파싱 | 마크다운 코드 블록(**` ```json `**)을 먼저 찾아내는 안정적인 방식 |
| **오류 처리** | 기본적인 예외 처리 | **HTTP 상태 오류**를 구체적으로 처리하여 안정성 강화 |
| **코드 구조** | 복잡하고 중복되는 로직 포함 | 단일화되고 간소화되어 **가독성 및 유지보수성 향상** |

- free-text


이번 변경은 단순한 수정이 아닌, **코드의 전체 구조를 완전히 재설계한 수준의 매우 큰 변화**입니다. 현재 코드는 복잡했던 기존 코드의 아키텍처를 버리고, 훨씬 **단순하고 직관적이면서도 더 스마트한 기능**을 갖춘 효율적인 구조로 변경되었습니다.

-----

### \#\# 1. LangGraph 아키텍처 제거 및 절차적 로직으로의 전환 🏛️

가장 큰 변화는 **`LangGraph` 기반의 복잡한 상태 머신(State Machine) 아키텍처를 완전히 제거**한 것입니다.

  * **기존 코드:**
    `LangGraph`를 사용하여 `validate_query`, `retrieve_context`, `generate_response` 등 각 단계를 '노드(Node)'로 정의하고, 이 노드들을 연결하여 대화의 흐름을 제어했습니다. 이는 유연하지만 구조가 복잡하고, 전체 흐름을 한눈에 파악하기 어려웠습니다.

  * **현재 코드:**
    `LangGraph`를 모두 제거하고, `process_chat`이라는 하나의 함수 안에서 **①세션 관리 → ②카테고리/위치/직업 변경 → ③RAG 검색 → ④LLM 호출 → ⑤결과 전송**의 흐름이 명확하게 순서대로 진행되는 **단순한 절차적 방식**으로 변경되었습니다. 이로 인해 구조가 훨씬 단순해지고 코드의 흐름을 이해하고 유지보수하기가 매우 쉬워졌습니다.

-----

### \#\# 2. 대화 기록(History) 관리 방식 표준화 📚

LLM에게 대화의 문맥을 전달하는 방식이 더 표준적이고 안정적으로 개선되었습니다.

  * **기존 코드:**
    대화 기록을 `"User: ..."`, `"Assistant: ..."` 와 같은 단순한 **문자열 리스트**로 저장했습니다.

  * **현재 코드:**
    `[{'role': 'user', 'content': '...'}, {'role': 'assistant', ...}]` 와 같이 **역할(role)과 내용(content)을 명확히 구분하는 딕셔너리 객체 리스트**로 저장합니다. 이는 OpenAI를 비롯한 대부분의 LLM이 표준으로 사용하는 형식으로, 모델이 대화의 문맥을 훨씬 더 정확하게 이해하도록 돕습니다.

-----

### \#\# 3. 위치/직업 자동 추론 기능 추가 (핵심 개선) ✨

사용자 경험을 크게 향상시키는 매우 중요한 기능이 추가되었습니다.

  * **기존 코드:**
    위치, 직업 정보는 최초에 `base-info`를 통해 입력받아야만 했습니다.

  * **현재 코드:**
    사용자가 자유롭게 대화하는 내용에서 **위치("바다", "사무실")나 직업("영업", "재택근무") 관련 키워드를 자동으로 감지**하여 세션 상태에 `location`과 `workType`을 업데이트합니다. 이를 통해 챗봇이 사용자의 상황을 스스로 추론하여 **더욱 개인화된 챌린지를 추천**할 수 있게 되었습니다.

-----

### \#\# 4. 스트리밍 및 파싱 로직 개선 ⚙️

`get_llm_response` 함수는 이전에 설명드렸던 개선 사항이 그대로 적용되었습니다.

  * **기존 코드:**
    복잡한 `word_delimiters` 기반의 비효율적인 스트리밍 로직을 사용했습니다.

  * **현재 코드:**
    **공백(space) 단위**의 효율적인 스트리밍, **챌린지 번호 앞 자동 줄바꿈**, **마크다운 코드 블록 기반의 안정적인 최종 파싱** 등 완전히 개선된 로직이 적용되었습니다.
...
| 기능 | 기존 코드 | 현재 코드 (개선) |
| :--- | :--- | :--- |
| **아키텍처** | `LangGraph` 기반의 복잡한 상태 머신 구조 | **단순한 절차적 구조**로 변경하여 가독성 및 유지보수성 향상 |
| **대화 기록 관리** | 단순 문자열 리스트로 저장 | `role`/`content`를 포함한 **표준 딕셔너리 객체**로 저장 |
| **핵심 로직** | 각 기능이 노드로 분산되어 복잡함 | `process_chat` 함수에 모든 로직이 **순차적으로 통합**되어 명확함 |
| **신규 기능** | 없음 | 대화 내용에서 **위치/직업을 자동으로 추론**하는 기능 추가 |
| **스트리밍/파싱** | 복잡하고 불안정한 로직 | 이전에 개선된 **안정적이고 효율적인 로직**이 그대로 적용됨 |
```